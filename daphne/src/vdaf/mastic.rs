...
mod test {
    ...
    async fn heavy_hitters_count() {
        ...
        // XXX Track ID rather than metadata? Forces report storage to be per report. This is fine
        // as long as aggregate store is still split by buckets.

        let pending_reports_stored = reports
            .iter()
            .map(|report| DapPendingReport::Stored(report.report_metadata.clone()))
            .collect::<Vec<_>>();
        ...
        for _ in 0..input_size * 8 {
            ...
            let (leader_encrypted_agg_share, helper_encrypted_agg_share) = {
                let (leader_state, agg_job_init_req) =
                    t.produce_agg_job_req(&agg_param, pending_reports).await;

                let (leader_agg_span, helper_agg_span) = {
                    let (helper_agg_span, agg_job_resp) =
                        t.handle_agg_job_req(agg_job_init_req).await;
