...
use crate::{
    ...
    vdaf::{
        // XXX Maybe just fold Prio2 into prio3, since they are so similar in shape?
        prio2::{
            prio2_encode_prepare_message, prio2_helper_prepare_finish, prio2_leader_prepare_finish,
            prio2_prepare_init, prio2_shard, prio2_unshard, Prio2Error,
        },
        prio3::{
...
impl VdafConfig {
    ...
    pub fn produce_report(
        ...
        let encoded_input_shares = match self {
            ...
            Self::Prio2 { input_len } => prio2_shard(*input_len, measurement)?,
        };
    ...
    pub(crate) fn consume_report_share<D: HpkeDecrypter>(
        ...
        match (self, verify_key) {
            ...
            (Self::Prio2 { input_len }, VdafVerifyKey::Prio2(ref verify_key)) => {
            ...
            agg_resp.transitions.into_iter().zip(state.seq.into_iter())
        {
            ...
            match self {
                ...
                Self::Prio2 { input_len } => {
                    ...
                    match prio2_leader_prepare_finish(
                        *input_len,
                        leader_step,
                        leader_message,
                        helper_message,
                        ...
                        //
                        // TODO Log the reason the report was skipped.
                        Err(Prio2Error::Codec(..)) | Err(Prio2Error::Vdaf(..)) => (),
                    }
                }
            };
        }
            ...
            for (helper_step, helper_nonce) in &mut helper_iter {
                ...
                let var = match self {
                    ...
                    Self::Prio2 { input_len } => {
                        ...
                        match prio2_helper_prepare_finish(*input_len, helper_step, leader_message) {
                            Ok(data) => {
                                let checksum = ring::digest::digest(
                                    &ring::digest::SHA256,
                                    &helper_nonce.get_encoded(),
    ...
    pub fn consume_encrypted_agg_shares<D: HpkeDecrypter>(
        ...
        match self {
            ...
            Self::Prio2 { input_len } => Ok(prio2_unshard(*input_len, agg_shares)?),
        }
    }
}
