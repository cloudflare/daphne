...
impl ParameterizedEncode<DapVersion> for ReportMetadata {
    fn encode_with_param(
        ...
        match version {
            DapVersion::Draft09 => {
                if self.public_extensions.is_some() {
                    return Err(CodecError::UnexpectedValue);
                }
            }
            DapVersion::Latest => match &self.public_extensions {
                Some(extensions) => encode_u16_items(bytes, version, extensions.as_slice())?,
                None => return Err(CodecError::UnexpectedValue),
            },
        }
