...
use crate::{
    error::DapAbort,
    messages::{
        AggregationJobInitReq, AggregationJobResp, PartialBatchSelector, ReadyAggregationJobResp,
        ReportError, ReportId, ReportMetadata, TaskId, TransitionVar,
    },
    metrics::ReportStatus,
    protocol::aggregator::ReportProcessedStatus,
    roles::{aggregator::MergeAggShareError, resolve_task_config},
    DapAggregateShare, DapAggregateSpan, DapError, DapRequest, DapTaskConfig, InitializedReport,
    WithPeerPrepShare,
};
use std::{
...
impl HandleAggJob<UniqueInitializedReports> {
    pub fn finish(
        ...
        task_config.produce_agg_job_resp(task_id, &Default::default(), &part_batch_sel, &reports)
    }
}
