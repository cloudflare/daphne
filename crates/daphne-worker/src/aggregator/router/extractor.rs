    ...
    async fn from_request(req: Request, state: &S) -> Result<Self, Self::Rejection> {
        ...
        let mtls_authed = if let Some(verification_result) = cf_tls_client_auth {
            ...
            match verification_result {
                TlsClientAuth::Success => {}
                TlsClientAuth::Failed(error) => {
                    return Err(auth_error(format!("Invalid TLS certificate ({error})")));
                }
            }
...
fn mtls_auth_from_request<B>(req: &http::Request<B>) -> Option<TlsClientAuth> {
    ...
    req.extensions()
        ...
        .map(|cert| {
            ...
            match verified.as_str() {
                "SUCCESS" => TlsClientAuth::Success,
                _ => TlsClientAuth::Failed(verified.to_string()),
            }
        })
}
...
mod test {
    ...
    use super::{
        dap_sender::FROM_LEADER,
        metrics::{AuthMethod, DaphneServiceMetrics},
        resource_parsers, DecodeFromDapHttpBody, UnauthenticatedDapRequestExtractor,
    };
        ...
        B::ResourceId: resource_parsers::Resource + Send,
    {
        ...
        impl<R: Send + Sync + RequestBody<ResourceId: Send + Sync>> super::DaphneService for Channel<R> {
            fn server_metrics(&self) -> &dyn DaphneServiceMetrics {
                // These tests don't care about metrics
                struct Metrics;
                impl DaphneServiceMetrics for Metrics {
                    fn abort_count_inc(&self, _label: &str) {}
                    fn count_http_status_code(&self, _status_code: u16) {}
                    fn daphne(&self) -> &dyn DaphneMetrics {
                        unimplemented!()
                    }
                    fn auth_method_inc(&self, _method: AuthMethod) {}
                    fn aggregate_job_latency(&self, _time: std::time::Duration) {}
                }
