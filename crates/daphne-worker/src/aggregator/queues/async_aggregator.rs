...
pub async fn async_aggregate_one(
        ...
        .transpose()
    {
        ...
        Err(e) => bail!(err = e, "taskprov advertisement was illformed"),
    };
    // 1.
    let (task_config, get_task_config_duration) = time_it(daphne::roles::resolve_task_config(
        app,
    ...
    let task_config = match task_config {
        Ok(t) => t,
        Err(e) => bail!(retry message, err = e, "not such task config"),
    };

    // 2.
    tracing::debug!("initializing reports");
    let (initialized_reports, initialize_reports_duration) = match time_it(
        app.compute_offload
    ...
    .await
    {
        ...
        (Err(e), _) => bail!(retry message, err = e, "failed to initialize reports"),
    };
    ...
    let state_machine = match state_machine {
        Ok(st) => st,
        Err(e) => bail!(retry message, err = e, "failed to check replays"),
    };
    ...
    let (span, agg_job_response) = match state_machine.finish() {
        ...
        Err(e) => bail!(err = e, "failed to finish aggregation"),
    };

    let ((), store_aggregate_share_duration) = time_it(async {
        for (bucket, (share, _)) in span {
            ...
            match response {
                Ok(()) => {}
                Err(e) => bail!(retry message, err = e, "failed to store aggregate share"),
            }
        }
    })
    .await;
    ...
    let (result, store_aggregate_job_response_duration) = time_it(
        app.durable()
            .with_retry()
            .request(
    ...
    match result {
        Ok(()) => {}
        Err(e) => bail!(retry message, err = e, "failed to store aggregation response"),
    }
...
pub async fn async_aggregate(app: App, message_batch: MessageBatch<()>) {
    tracing::info!(
    ...
    message_batch
        ...
        .map(|m| async move {
            let message = match deserialize(&m) {
                Ok(m) => m,
                Err(e) => bail!(err = e, "failed to deserialize replay queue message"),
            };
...
async fn time_it<F>(future: F) -> (F::Output, Duration)
where
    F: Future,
{
