...
async fn handle_test_routes(action: TestAction, http_client: HttpClient) -> anyhow::Result<()> {
    match action {
        ...
        TestAction::CreateAddTaskJson {
            ...
            helper_url,
            vdaf,
            leader_auth_token: leader_authentication_token,
            ...
            let internal_task = InternalTestAddTask {
                ...
                collector_authentication_token: match role {
                    DapRole::Leader => use_or_request_from_user_opt(
                        collector_authentication_token,
...
fn use_or_request_from_user_opt<T>(value: Option<T>, field: &'static str) -> io::Result<Option<T>>
where
    T: FromStr + fmt::Display,
    T::Err: fmt::Debug,
{
    match value {
        Some(v) => Ok(Some(v)),
        None => {
            request_from_user::<T>(field, "leave empty to default", || ControlFlow::Break(None))
